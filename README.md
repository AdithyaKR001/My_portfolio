...
#### **ii.	Incident-RAG: Grounded incident-resolution assistant:**

* Development of a production-style RAG (Retrieval-Augmented Generation) service over a large DevOps knowledge base (resolved incidents + runbooks). Given a failure question in natural language, it retrieves the most relevant documents, answers only from that context, and cites the document ids it used. Pairs with the Agentic DevOps Copilot above as the retrieval/grounding layer that agent would call to stop reasoning from scratch. Runs fully locally on Ollama, no API key required.
...
* Other technologies used:
    * Ollama: local chat (Llama 3.1) and embedding (nomic-embed-text) models.
    * Chroma: vector store for the embedded document corpus.
    * Docker & Docker Compose: one-command run of the full stack (ingest, API, chat UI, dashboard, Ollama).
    * GitHub Actions: CI workflow that runs the deterministic test/eval suite.

* Architecture:
    * End-to-end RAG pipeline: ingest → chunk → embed → vector store (Chroma) → retrieve top-k → optional lexical rerank → grounded generation with citations.
    * Interfaces: a CLI, a FastAPI REST API (`POST /query`), and a Streamlit chat UI showing citations and retrieved context.
    * A separate Streamlit metrics dashboard visualizes evaluation results.
    * Fully env-driven tuning (`TOP_K`, `CHUNK_SIZE`, `CHUNK_OVERLAP`, `RERANK`) for a measure-tune-remeasure workflow.

* Datasets used:
    * Synthetic, reproducible corpus generated by a data-generation script: ~180 resolved incidents (symptoms, root cause, resolution) across 10 failure categories and 10 services, one runbook per category, plus ~40 labelled golden Q&A pairs for evaluation.
